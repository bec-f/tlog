                ...
                const struct tlog_pkt_pos *end)
{
    ...
    if(rate_limit_sink->prev_pkt->type != TLOG_PKT_TYPE_IO || rate_limit_sink->rate == 0){
        ...
        uint64_t time_change = (pkt->timestamp.tv_sec - rate_limit_sink->prev_pkt->timestamp.tv_sec) * 1000000
                                + (pkt->timestamp.tv_nsec - rate_limit_sink->prev_pkt->timestamp.tv_nsec)/1000;
        ...
        char buf[20];
        strftime(buf, sizeof(buf), "%b %d %T", timer);
        fprintf(f, "Sink write, looking at %d bytes: %s\n", rate_limit_sink->prev_pkt->data.io.len, rate_limit_sink->prev_pkt->data.io,buf);
        fprintf(f, "For above, sink-rate= %lu, cur-rate=%lu; [%s]\n", rate_limit_sink->rate, cur_rate, buf);
        fclose(f);
        //END TRACING CODE
